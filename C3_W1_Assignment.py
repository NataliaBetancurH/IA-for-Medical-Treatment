...
def event_rate(df):
    ...
    # Verificar si hay pacientes tratados antes de calcular la probabilidad tratada
    if sum(df.TRTMT == 1) > 0:
        treated_prob = np.float64(sum((df.TRTMT == 1) & (df.outcome == 1)) / sum((df.TRTMT == 1)))
    
    # Verificar si hay pacientes no tratados antes de calcular la probabilidad no tratada
    if sum(df.TRTMT == 0) > 0:
        control_prob = np.float64(sum((df.TRTMT == 0) & (df.outcome == 1)) / sum((df.TRTMT == 0)))
